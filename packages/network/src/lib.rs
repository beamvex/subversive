...
pub async fn setup_network(
    port: u16,
    host: Option<String>,
) -> Result<(u16, Vec<Gateway2>, String)> {
    // Get external IP and resolve hostname
    let own_address = format!("{}:{}", host.unwrap_or_default(), port);
...
mod tests {
    ...
    async fn test_setup_network_with_hostname() -> Result<()> {
        ...
        let hostname = Some("test.example.com".to_string());
        ...
        // Call setup_network
        let (actual_port, _gateways, address) = setup_network(port, hostname).await?;
    ...
    async fn test_setup_network_without_hostname() -> Result<()> {
        ...
        // Cal l setup_network
        let port = 12345;
        let (actual_port, _gateways, address) = setup_network(port, None).await?;
